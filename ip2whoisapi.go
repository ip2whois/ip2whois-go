...
func (a *Api) LookUp(domain string) (IP2WhoisResult, error) {
	...
	myUrl := "https://" + baseURL + "?key=" + url.QueryEscape(a.apiKey) + "&domain=" + url.QueryEscape(domain) + "&source=" + url.QueryEscape(source) + "&source_version=" + url.QueryEscape(sourceVersion)
...
func (a *Api) GetDomainName(fullURL string) (string, error) {
	...
	host, err := idna.ToASCII(parsedURL.Host)
	if err != nil {
		return "", err
	}

	for i := 0; i < len(tlds); i++ {
		if strings.HasSuffix(host, tlds[i]) {
			tmp := strings.TrimSuffix(host, tlds[i])
			domain := host[strings.LastIndex(tmp, ".")+1:]
			return domain, nil
		}
	}
...
func (a *Api) GetDomainExtension(str string) (string, error) {
	domain, err := a.GetDomainName(str)

	if err != nil {
		return "", err
	}

	ext := domain[strings.Index(domain, "."):]
